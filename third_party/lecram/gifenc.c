 ...
 * pixels is a w*h array of palette indices (row-major). */
void
ge_add_frame_at(ge_GIF *gif, uint16_t delay,
                ...
                uint16_t w, uint16_t h)
{
    ...
    if (delay || (gif->bgindex >= 0))
        add_graphics_control_extension(gif, delay);
    /* blit pixel data into the canvas buffer at the specified position */
    for (i = 0; i < h; i++) {
        for (j = 0; j < w; j++) {
            gif->frame[(y + i) * gif->w + x + j] = pixels[i * w + j];
        }
    }
    put_image(gif, w, h, x, y);
    gif->nframes++;
